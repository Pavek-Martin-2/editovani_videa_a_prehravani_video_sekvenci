...
# upraveno 11.7.2026
$radek_mpv_3b = "REM tady si jeste muzete vybrat, budto jedno nebo druhy"
$pole_prehravani_v_mpv += $radek_mpv_3b

$radek_mpv_4 = "REM set f=" + '"' + "no" + '"'
$pole_prehravani_v_mpv += $radek_mpv_4
$pole_prehravani_v_mpv += ""

$radek_mpv_5 = "set m=" + '"' + "no" +'"'
$pole_prehravani_v_mpv += $radek_mpv_5

$radek_mpv_6 = "REM set m=" + '"' + "yes" +'"'
...
# 17.6.2026 pridana kontrola radku casoveho useku od-do pomoci regulerniho vyrazu
# definice regularniho vyrazu, pro kontrolu radku casoveho useku od-do
$kontrola = '^\d{2}:\d{2}:\d{2}\.\d{3}\s+\d{2}:\d{2}:\d{2}\.\d{3}$'
...
# Testování pomocí podmínky IF a operátoru -notmatch (neodpovida)
if ( $radek_seznamu -notmatch $kontrola ) {
Write-Warning "chyba, tento radek neodpovida pozadovanmu formatu casoveho useku"
Write-Warning "neplatny radek: $radek_seznamu"
Write-Host -ForegroundColor Red "opravte v souboru $nazev_seznamu tento radek a program spuste znovu"
sleep 10
exit
}
...
$radek_mpv_11 = "mpv --fullscreen=%f% --mute=%m% --speed=%sp% --start=%" + "a" + $poc_2 + "%"
...
#echo $rozdil_vterin_out.GetTypeCode()

# 17.6.2026 pridana kontrola na zaporne cislo ( prvni cas casoveho useku je mensi nezli druhy)
$kontrola_zaporne_cislo = $rozdil_vterin_out.Contains("-") # True kdyz je zaporne
if ( $kontrola_zaporne_cislo -like "True" ) {

Write-Warning "chyba neplatny radek: $radek_seznamu "
Write-Warning "prvni cas casoveho useku od-do je vetsi nezli druhy"
Write-Host -ForegroundColor Red "opravte v souboru $nazev_seznamu tento radek a program spuste znovu"
sleep 10
exit
}
